...
<FormControl defaultValue={todo.id} className="w-50" onChange={(e) => setTodo({ ...todo, id: e.target.value })}/><hr/>
      ...
      <br /><br /><hr />   
      <h3>Completing an Array</h3>
<a id="wd-completed-todo" className="btn btn-primary float-end" href={`${API}/${todo.id}/complete`}>
   Mark Todo as Complete </a>
    <FormControl type="checkbox" className="form-check-input me-2" defaultChecked={todo.completed} onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
        setTodo({ ...todo, completed: e.target.checked })
    }/><hr />
<h3>Description</h3>
<a id="wd-description-todo" className="btn btn-primary float-end" href={`${API}/${todo.id}/description/${todo.description}`}>
...
<FormControl defaultValue={todo.description} className="w-50 float-start" onChange={(e) => setTodo({ ...todo, description: e.target.value })}/><hr/>
    </div>
);}
